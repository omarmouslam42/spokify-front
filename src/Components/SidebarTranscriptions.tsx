...
export const SidebarTranscriptions = forwardRef(({ userId, onSelect }: any, ref) => {
  const [transcriptions, setTranscriptions] = useState([]);

  const fetchTranscriptions = async () => {
    try {
      const res = await axios.get(`http://127.0.0.1:4000/api/transcriptions/user/${userId}`);
    ...
    <div className="bg-white/10 p-4 rounded-lg shadow-md">
      ...
      <ul className="space-y-2 max-h-[300px] overflow-y-auto">
        {transcriptions.map((item: any) => (
          ...
          >
            {item.metadata?.filename || "Untitled"}
          </li>
        ))}
      </ul>
    </div>
